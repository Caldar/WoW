...
function AutoEquip:TryToEquipItem()
	...
	new_score = ItemScore:GetItemScore(item.link,item.equipslot,nil,self.verbose)
	if self.CurrentGear[item.slot] then
		ItemScore.__ItemCache[self.CurrentGear[item.slot].link]=nil
		old_score = ItemScore:GetItemScore(self.CurrentGear[item.slot].link,self.CurrentGear[item.slot].slot,nil,self.verbose)
	end
