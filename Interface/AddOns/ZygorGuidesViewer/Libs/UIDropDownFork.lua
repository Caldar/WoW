...
local function UIDropDownForkDelegate_OnAttributeChanged (self, attribute, value)
...
function UIDropDownFork_AddSeparator(info, level)
	info.text = nil;
	info.hasArrow = false;
	info.dist = 0;
	info.isTitle = true;
	...
	UIDropDownFork_AddButton(info, level);
end
...
function UIDropDownFork_AddButton(info, level)
	...
	width = max(UIDropDownFork_GetButtonWidth(button), info.minWidth or 0);
	...
	listFrame:SetHeight((index * UIDROPDOWNFORK_BUTTON_HEIGHT) + (UIDROPDOWNFORK_BORDER_HEIGHT * 2));
		...
		if ( level == 1 ) then	
			...
			if ( not anchorName ) then
				...
				if ( dropDownFrame.relativeTo ) then
					...
					relativeTo = UIDROPDOWNFORK_OPEN_MENU:GetName().."Left";
				end
