
Postal3DB = {
	["global"] = {
		["BlackBook"] = {
			["alts"] = {
				"Caladyth|Feathermoon|Alliance|92|MAGE", -- [1]
				"Calastir|Feathermoon|Alliance|70|ROGUE", -- [2]
				"Calastir|Kirin Tor|Horde|91|WARLOCK", -- [3]
				"Calastir|Scarlet Crusade|Alliance|100|HUNTER", -- [4]
				"Caldar|Feathermoon|Alliance|100|PRIEST", -- [5]
				"Censure|Feathermoon|Alliance|90|DEATHKNIGHT", -- [6]
				"Censure|Kirin Tor|Horde|55|DEATHKNIGHT", -- [7]
				"Failure|Feathermoon|Alliance|100|PALADIN", -- [8]
				"Nothing|Feathermoon|Alliance|100|MONK", -- [9]
				"Pyric|Feathermoon|Alliance|75|WARRIOR", -- [10]
				"Sengchou|Feathermoon|Alliance|90|MONK", -- [11]
				"Sinister|Feathermoon|Alliance|100|SHAMAN", -- [12]
				"Souptruck|Feathermoon|Alliance|100|DRUID", -- [13]
				"Zenax|Scarlet Crusade|Alliance|85|WARLOCK", -- [14]
			},
		},
	},
	["profileKeys"] = {
		["Caldar - Feathermoon"] = "Caldar - Feathermoon",
		["Calastir - Feathermoon"] = "Calastir - Feathermoon",
		["Failure - Feathermoon"] = "Failure - Feathermoon",
		["Zenax - Scarlet Crusade"] = "Zenax - Scarlet Crusade",
		["Calastir - Scarlet Crusade"] = "Calastir - Scarlet Crusade",
		["Caladyth - Feathermoon"] = "Caladyth - Feathermoon",
		["Censure - Kirin Tor"] = "Censure - Kirin Tor",
		["Calastir - Kirin Tor"] = "Calastir - Kirin Tor",
		["Pyric - Feathermoon"] = "Pyric - Feathermoon",
		["Nothing - Feathermoon"] = "Nothing - Feathermoon",
		["Sengchou - Feathermoon"] = "Sengchou - Feathermoon",
		["Sinister - Feathermoon"] = "Sinister - Feathermoon",
		["Souptruck - Feathermoon"] = "Souptruck - Feathermoon",
		["Censure - Feathermoon"] = "Censure - Feathermoon",
	},
	["profiles"] = {
		["Caldar - Feathermoon"] = {
			["BlackBook"] = {
				["recent"] = {
					"Sinister|Feathermoon|Alliance", -- [1]
					"Nothing|Feathermoon|Alliance", -- [2]
				},
			},
		},
		["Calastir - Feathermoon"] = {
		},
		["Failure - Feathermoon"] = {
			["BlackBook"] = {
				["recent"] = {
					"Nothing|Feathermoon|Alliance", -- [1]
				},
			},
		},
		["Zenax - Scarlet Crusade"] = {
		},
		["Calastir - Scarlet Crusade"] = {
		},
		["Caladyth - Feathermoon"] = {
		},
		["Censure - Kirin Tor"] = {
		},
		["Calastir - Kirin Tor"] = {
		},
		["Pyric - Feathermoon"] = {
		},
		["Nothing - Feathermoon"] = {
		},
		["Sengchou - Feathermoon"] = {
		},
		["Sinister - Feathermoon"] = {
		},
		["Souptruck - Feathermoon"] = {
		},
		["Censure - Feathermoon"] = {
		},
	},
}
